...
will pass through the controller again... :)
    ...
    let $xbelfilename := local:get-filename-without-extension($exist:resource)
    return 
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <forward url="{concat($exist:controller, '/data/',$xbelfilename,'.xml')}"/>
        <view>
            <forward servlet="XSLTServlet">
                <set-attribute name="xslt.stylesheet" value="{concat($exist:root, $exist:controller, "/xsl/xbel.xsl")}"/>
                <set-attribute name="xslt.description" value="{request:get-parameter('description','yes')}"/>
            </forward>
        </view>
    </dispatch>  
